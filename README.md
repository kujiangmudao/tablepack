...
**PDF tables �?multi-sheet Excel packages**, powered by [MinerU](https://github.com/opendatalab/MinerU), with **original table screenshots**, figure assets, and **honest QC notes**.
...
Built from multi-batch production experience (complex Chinese theses, multi-level headers, empty MinerU nodes, cross-page tables, OCR oxide formulas like `Al₂O₃`, etc.).
...
| `--keep-empty-tables` | Keep empty `table_body` nodes (default: drop, no fake data) |
...
Automation gets you ~70�?0% of the way. **Deliverable quality** needs visual QC:
...
For Cursor / Claude / Grok / other agents:
...
- �?Ship copyrighted sample PDFs (use your own)
- �?Invent table rows when MinerU returns empty HTML
- �?Claim �?00% accurate�?without visual QC
